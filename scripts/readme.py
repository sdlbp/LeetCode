...
class Config:
    ...
    """
    local_path = u'/Users/lbp/Documents/个人/个人代码库/LeetCode.git'
    # solution of leetcode
    github_leetcode_url = 'https://github.com/sdlbp/LeetCode/blob/master/leetcode-algorithms/'
                ...
                for _, _, files in os.walk(os.path.join(oj_algorithms, folder)):
                    ...
                    for item in files:
                        ...
                        if item.endswith('.py'):
                            ...
                            complete_info.solved['swift'] += 1
                            folder_url = folder.replace(' ', "%20")
                            folder_url = os.path.join(folder_url, item)
                            folder_url = os.path.join(Config.github_leetcode_url, folder_url)
                            # print(folder_url)
                            self.table_item[folder[:3]].swift = '[swift]({})'.format(folder_url)
...
class Readme:
    ...
    def create_leetcode_readme(self, table_instance):
        ...
        with open(file_path, 'a') as f:
            ...
            f.write('| ID | Title | Difficulty | JavaScript | Python | C++ | Java |swift|\n')
            f.write('|:---:' * 7 + '|\n')
